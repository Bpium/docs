...
Выберите поле, по которому данные будут разбиваться на группы.&#x20;
...
* Сумма сделок (значение) → по менеджерам (ось) → разбить по статусам
* Число обращений (значение) → по месяцам (ось) → разбить по звёздам качества

#### **5. «Сложить значения»**

Включает режим, при котором значения, разложенные по полю «разбить по», располагаются друг над другом (Stacked), а не независимо рядом. Также эта опция фиксирует нижнюю границу оси Y в нуле. При выключенной опции нижняя граница определяется автоматически.
