...
Сценарии в Бипиуме строятся из связанных между собой компонентов. Для описания логики используется нотация **BPMN 2.0** -- стандарт, понятный как людям, так и системе. Компоненты обмениваются данными через переменные.
...
-  [Начало процесса](./sistemnye/nachalo-processa)

-  [Конец процесса](./sistemnye/konec-processa)

-  [Таймер](./sistemnye/taimer)
...
-  [Получить запись](./deistviya-s-dannymi/getrecord)

-  [Найти записи](./deistviya-s-dannymi/findrecords)

-  [Изменить запись](./deistviya-s-dannymi/editrecord)

-  [Создать запись](./deistviya-s-dannymi/createrecord)

-  [Удалить запись](./deistviya-s-dannymi/deleterecord)

-  [Структура каталога](./deistviya-s-dannymi/poluchenie-struktury-kataloga)

-  [Загрузить файл](./deistviya-s-dannymi/zagruzit-fail)

-  [Сгенерировать документ](./deistviya-s-dannymi/generaciya-dokumenta)

#### Действия

-  [Назначение переменных](./deistviya/setvariables)

-  [Код (JavaScript)](./deistviya/code)

-  [Веб-запрос](./deistviya/webrequest)

-  [SQL-запрос](./deistviya/sql)

-  [Парсер](./deistviya/parse)

-  [Запуск процесса](./deistviya/zapusk-processa)

-  [Отправка почты](./kommunikacii/email)
...
-  [Шлюз ИЛИ](./sistemnye/shlyuz-ili-uslovnoe-vetvlenie).
...
Условия могут быть заданы только на соединяющие линии, выходящие из компонента [«Шлюз ИЛИ»](./sistemnye/shlyuz-ili-uslovnoe-vetvlenie).

## Свойства

**Условие**  \
Логическое выражение, которое возвращает true/false. Логические выражения задаются в формате [выражений](./../scripts-2/expression).
...
Входные параметры поступают на вход компонента. Значения входных параметров можно задавать явными значениями (числа и "строки"), указывать [переменные](./../events/scripts/variables) или [выражения](./../events/scripts/expression).
