...
| Поле                | Описание                                                                                                                                                                                                                |
|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Метод               | Тип HTTP-запроса: `GET`, `POST`, `PATCH`, `PUT`, `DELETE`, `HEAD` (по умолчанию `GET`)                                                                                                                                  |
| Адрес (URL)         | Адрес, на который отправляется запрос. Может содержать GET-параметры. Формат: текст в кавычках или выражение                                                                                                            |
| Заголовки (headers) | Дополнительные заголовки HTTP-запроса. Формат: список пар «заголовок = значение / выражение». Если указать заголовок, который уже задан системой (например, `Content-Type`), будет использовано указанное вами значение |
...
| Формат        | Описание                                                                                                                                                   |
|---------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Ключ/значение | Параметры передаются как форма (`application/x-www-form-urlencoded`). Появляется поле «Запрос (параметры)» -- список пар «параметр = значение / выражение» |
| Тело запроса  | Данные передаются в теле запроса. Появляются поля: «Запрос (body)» (текст или выражение) и «Тип данных» (`JSON`, `XML`, `SOAP`, `Plain text`)              |
...
| OAuth2                      | Появляется поле «Токен» -- можно выбрать или создать шаблон авторизационных ключей из каталога «[Доступы к сервисам](./../../../systemcatalogs/upravlenie/oauthservices)» |
