...
2. **Сохранить в Выходной параметр.** Сохраняет ответ в указанную переменную. В зависимости от значения Результат в качестве ответа компонент возвращает строку, JSON-объект или весь объект с параметрами от API. Пример:
