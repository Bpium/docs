---
description: >-
  Бипиум — браузерное приложение. Всё что нужно для работы — это браузер и
  доступ в интернет. Интерфейс состоит из нескольких постоянных зон и рабочей
  области в центре.
---

# Интерфейс системы

## Структура интерфейса

<table><thead><tr><th width="227">Зона/номер на скриншоте</th><th>Что содержит</th></tr></thead><tbody><tr><td><a href="sections.md#verkhnyaya-panel"><strong>Верхняя панель</strong></a> <strong>/ 1</strong></td><td>Логотип, меню разделов, иконка чата, подсказки по работе.</td></tr><tr><td><a href="sections.md#menyu-katalogov"><strong>Меню каталогов</strong></a> <strong>/ 2</strong></td><td>Каталоги открытого раздела и кнопка создания каталога.</td></tr><tr><td><a href="sections.md#levaya-panel"><strong>Левая панель</strong></a> <strong>/ 3</strong></td><td>Виды, фильтры, поля и раскладка для текущего каталога.</td></tr><tr><td><a href="sections.md#rabochaya-oblast"><strong>Рабочая область</strong></a> <strong>/ 4</strong></td><td>Записи каталога в выбранном режиме отображения.</td></tr><tr><td><a href="sections.md#menyu-polzovatelya"><strong>Меню пользователя</strong></a> <strong>/ 5</strong></td><td>Смена языка, имя, почта, настройки фона и т.п.</td></tr></tbody></table>

<figure><img src="../.gitbook/assets/Интерфейс база.png" alt=""><figcaption><p>Общий интерфейс системы</p></figcaption></figure>

## Верхняя панель

Расположена в самом верху экрана и видна всегда, в каком бы каталоге вы ни находились.

### **Логотип Бипиум**

Нажав на логотип, вам откроется дополнительное меню с разделами и каталогами. В котором вы можете быстро найти необходимый вам каталог или добавить новый раздел.

<figure><img src="../.gitbook/assets/Бипиум логотип стрелка.png" alt=""><figcaption><p>Логотип Бипиум.</p></figcaption></figure>

<figure><img src="../.gitbook/assets/Логотип бипиум после нажатия.png" alt=""><figcaption><p>Дополнительное меню разделов и каталогов.</p></figcaption></figure>

### Меню разделов

Разделы — это способ организации каталогов по темам или подразделениям компании. Например, «Логистика», «Туроператор», «Управление». Каждый раздел содержит набор своих каталогов. Нажмите на название раздела в верхней панели — ниже откроются его каталоги.

<figure><img src="../.gitbook/assets/Разделы.png" alt=""><figcaption></figcaption></figure>

{% hint style="info" %}
Вы видите только те разделы, к которым у вас есть доступ. Если нужного раздела нет — обратитесь к администратору системы.
{% endhint %}

### Панель чатов

Нажмите на иконку чата в верхней панели — откроется список записей с активными чатами. Записи отсортированы по времени последнего сообщения — самые свежие сверху. Нажмите на запись чтобы сразу открыть её карточку с чатом.

<figure><img src="../.gitbook/assets/Чатик.png" alt=""><figcaption><p>Иконка чата в интерфейсе</p></figcaption></figure>

<figure><img src="../.gitbook/assets/чатик2.png" alt=""><figcaption><p>Открытая панель чатов — список записей с последними сообщениями</p></figcaption></figure>

### Иконка подсказки

Нажмите на иконку подсказки — откроется поисковое окно с подсказками по работе в Бипиум.

<figure><img src="../.gitbook/assets/подсказки1 (1).png" alt=""><figcaption><p>Иконка подсказки в интерфейсе</p></figcaption></figure>

<figure><img src="../.gitbook/assets/подсказки2.png" alt=""><figcaption><p>Поисковое окно подсказок по работе в Бипиум</p></figcaption></figure>

## Меню каталогов

Находится под верхней панелью. Показывает все каталоги выбранного раздела в виде вкладок.

### **Вкладки каталогов**

Информация в Бипиуме структурирована. Записи с одинаковой структурой хранятся в каталогах. Например, каталоги клиентов, товаров, обращений, сделок, задач или проектов. В Бипиуме может быть неограниченное количество каталогов и записей в каждом из них. \
\
Нажмите на вкладку чтобы открыть нужный каталог. Активная вкладка выделяется белым цветом.

<figure><img src="../.gitbook/assets/каталоги.png" alt=""><figcaption><p>Вкладки каталогов</p></figcaption></figure>

### **Создать каталог**

Кнопка для добавления нового каталога в раздел. Видна только администраторам.

<figure><img src="../.gitbook/assets/создать каталог.png" alt=""><figcaption><p>Кнопка «+Создать каталог»</p></figcaption></figure>

## Левая панель

Расположена слева от рабочей области. Содержит инструменты для управления отображением записей в текущем каталоге.

Панель переключается между четырьмя режимами:

### Виды

Переключение между сохранёнными видами каталога. Каждый вид — это сохранённый набор фильтров, сортировок и отображаемых полей.

<figure><img src="../.gitbook/assets/виды (1).png" alt=""><figcaption></figcaption></figure>

### **Фильтры**

Список всех полей каталога с возможностью задать условия фильтрации. Активные фильтры подсвечиваются.

<figure><img src="../.gitbook/assets/фильтры (2).png" alt=""><figcaption></figcaption></figure>

### **Поля**

Управление отображаемыми столбцами и их порядком.

<figure><img src="../.gitbook/assets/Поля (3).png" alt=""><figcaption></figcaption></figure>

### **Раскладка**

Настройка параметров режимов «Таблица» и «Плитка» — группировка, сортировка карточек, цветовая подсветка.

<figure><img src="../.gitbook/assets/раскладка (1).png" alt=""><figcaption></figcaption></figure>

## Рабочая область

Основная часть экрана — здесь отображаются записи каталога. В верхней части рабочей области находится строка управления каталогом.<br>

<figure><img src="../.gitbook/assets/Рабочая область.png" alt=""><figcaption><p>Рабочая область — строка управления сверху и записи ниже</p></figcaption></figure>

### Строка управления каталогом

<table><thead><tr><th width="227">Зона / номер на скриншоте</th><th>Что это</th></tr></thead><tbody><tr><td><strong>Название каталога / 1</strong></td><td>Названия текущего каталога.</td></tr><tr><td><strong>Иконка шестерёнки / 2</strong></td><td>Настройки и права доступа к каталогу.</td></tr><tr><td><strong>Переключатель вида / 3</strong></td><td>Название текущего вида и кнопка выбора другого.</td></tr><tr><td><strong>Режимы отображения / 4</strong></td><td>Варианты режимов отображения записей каталога.</td></tr><tr><td><strong>Счётчик записей / 5</strong></td><td>Количество записей с учётом активных фильтров.</td></tr><tr><td><strong>Обновить /</strong> <strong>6</strong></td><td>Кнопка для обновления списка записей.</td></tr><tr><td><strong>+ Создать /</strong> <strong>7</strong></td><td>Кнопка для добавления новой записи.</td></tr><tr><td><strong>Поиск /</strong> 8</td><td>Быстрый поиск по текстовым полям.</td></tr></tbody></table>

<figure><img src="../.gitbook/assets/Строка управления каталогом.png" alt=""><figcaption><p>Строка управления с обозначенными элементами</p></figcaption></figure>

### Режимы отображения

Записи каталога можно просматривать в разных режимах — переключайтесь между ними в зависимости от задачи:

<table><thead><tr><th width="164">Режим</th><th>Для чего</th></tr></thead><tbody><tr><td><strong>Таблица</strong></td><td>Список записей в виде таблицы — основной режим работы</td></tr><tr><td><strong>Плитка</strong></td><td>Аналог kanban-доски для управления задачами по статусам</td></tr><tr><td><strong>Отчёты</strong></td><td>Графики и дашборды — анализ данных</td></tr><tr><td><strong>Сводка</strong></td><td>Сводная таблица с группировкой и подсчётами</td></tr><tr><td><strong>Подсчёт</strong></td><td>Быстрый личный анализ показателей</td></tr><tr><td><strong>Активность</strong></td><td>Лента изменений — кто и что менял в каталоге</td></tr><tr><td><strong>Инструкция</strong></td><td>Правила работы с каталогом от администратора</td></tr></tbody></table>

## Меню пользователя

**Меню пользователя** находится в правом верхнем углу интерфейса. При нажатии на аватар или имя текущего пользователя открывается панель со следующими элементами:

<table data-header-hidden><thead><tr><th width="202">Элемент</th><th>Назначение</th></tr></thead><tbody><tr><td><strong>Имя и почта</strong></td><td>Отображение данных авторизованного пользователя</td></tr><tr><td><strong>«Фон для вас»</strong></td><td>Настройка цветовой темы интерфейса</td></tr><tr><td><strong>Лицензия</strong></td><td>Информация по лицензии</td></tr><tr><td><strong>«Документация»</strong></td><td>Переход к официальной документации</td></tr><tr><td><strong>«Задать вопрос»</strong></td><td>Отправка запроса в техподдержку</td></tr><tr><td><strong>«Выйти»</strong></td><td>Завершение сессии</td></tr></tbody></table>

<figure><img src="../.gitbook/assets/Меню пользователястрелка (2).png" alt=""><figcaption><p>Меню пользователя </p></figcaption></figure>

<figure><img src="../.gitbook/assets/Меню пользователя открытое.png" alt=""><figcaption><p>Меню пользователя развернутое</p></figcaption></figure>

**Переключатель языка** (русский / английский) вынесен отдельно и находится рядом с меню пользователя.

<figure><img src="../.gitbook/assets/смена языка.png" alt=""><figcaption><p>Кнопка переключения языка в системе</p></figcaption></figure>
