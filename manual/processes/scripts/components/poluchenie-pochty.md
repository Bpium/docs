...
****\
****Вложения: Не получать При выборе этого значения вложения получаемого письма не будут получены.\
\
Вложения: Получить ссылки При выборе этого значения вложения получаемого письма будут получены в виде массива с файлами. Формат вложений:
...
* Пароль: пароль от эл. почты или пароль приложения (в кавычках)
...
Для возможности получать письма через протокол IMAP нужно дать соответствующий доступ в настройках вашего почтового ящика. Настройки доступов для сервисов Я.Почта и Gmail описаны ниже.
...
С 30 мая 2022 года Google не поддерживает вход на почту через сторонние приложения с использование логина и пароля от почтового ящика. Для того, чтобы получать сообщения из Gmail необходимо настроить в вашем аккаунте двухэтапную аутентификацию (2FA) и получить пароль приложения. Используя этот пароль в компоненте «Получения почты» можно получать сообщения в Бипиум.
