...
В настройках полей каталога можно перенести каталог между отделами. В верхней части левой панели необходимо выбрать нужный отдел из списка всех отделов. Перемещение каталога происходит без потери данных.
...
Бипиум предлагает на выбор более 1000 иконок. Иконки разделены по категориям.
...
Настройка производится в панели параметров поля, внутри настройки полей каталога. Выбираем поле, для которого хотим добавить условие видимости

![](<../../../.gitbook/assets/изображение (3).png>)

Далее переходим в параметры этого поля в секцию "Условия видимости поля" и выбираем поле(я) и его(их) значения при котором(ых) поле будет видимо.&#x20;
...
Объединяет другие поля карточки в визуальные группы. В анкете может быть множество секций, но должен быть как минимум один. Это поле имеет только название.
...
Поле для хранения контактных данных: телефонов, адресов электронной почты и сайтов. Позволяет создавать несколько контактов одного типа в одном поле. Например, несколько телефонов, каждый из которых со своим описанием.
...
Для того, чтобы получить токен необходимо зарегистрироваться на [Dadata.ru](https://dadata.ru/clean/#registration\_popup). В [Личном кабинете](https://dadata.ru/profile/#info) в разделе «Ключи и домены» сгенерировать API-ключ (токен). Полученный ключ ввести в поле «Токен сервиса Dadata.ru» в Бипиуме.
...
Если нет необходимости вводить полный и точный адрес, то можно использовать только обычное поле для заполнения адреса.
...
Поле схожее по смыслу с «категорией». Позволяет выбирать несколько значений. Подходит для отображения этапов, статусов, чек-листов. В режиме просмотра анкеты варианты значений показаны списком в виде галочек с подписями.
...
Поле схожее по смыслу с «категорией». Позволяет выбирать только одно из возможных значений. Подходит для отображения вопросов в анкетах. В режиме просмотра анкеты варианты значений показаны списком в виде радио-группы.
...
Расширенные (дополнительные) **поля** позволяют отобразить необходимые поля выбранного каталога. Например, ФИО, должность и телефон из карточки «Контакты».

**Права.** Редактирование расширенных полей связанных записей из карточки родительской записи позволяет не только видеть данные поля, но и изменять. Например, при заполнении и изменении контактов сотрудников, либо позиций или количества заказов компании.

**Галочка «Можно выбирать несколько значений»** включает возможность одновременно привязывать несколько записей других каталогов. Например, выбрать в участниках встречи несколько контактных лиц.
...
**Галочка «Можно выбирать несколько сотрудников»** включает возможность одновременно закреплять несколько сотрудников сразу.

**Галочка «По умолчанию: текущий сотрудник»** устанавливает в поле имя сотрудника, создающего новую запись.
