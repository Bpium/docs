...
-  `recordsFilter` (object) -- фильтры для выборки записей каталога, такие как filter, sort и другие. подробнее в описании параметров [получения записей](./../data/records#poluchit-zapisi).
...
В версии Бипиума 1.7.1 появился специальный ресурс [Values](./../agregate/values) (используйте его).\
...
-  *параметры аналогичны параметрам получения значений графика* [*Values*](./widgets#znacheniya-grafika-values)
...
-  *Параметры аналогичны параметрам получения значений разложения* [*Values*](./../agregate/values)*.*\
   *Игнорируются параметры: sort, sortType, limit, offset.*

### Получить общие данные без создания графика

В версии Бипиума 1.7.1 появился специальный ресурс [Totals](./../agregate/totals) (используйте его).\
В более ранних версиях используйте метод получения через существующий Дашборд:
