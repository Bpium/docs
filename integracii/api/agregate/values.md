...
description: >-
  ...
  критериям.
title: Разложения (Values)
order: 0.5
searchPhrases:
  - распределение записей
  - подсчет по критериям
  - тип подсчитываемого значения
  - поле для расчета
  - подтип значения
  - функция подсчета
  - группировка по дате
...
В предыдущих версиях Бипиума значения разложения можно получить через ресурс [Widget/Values](./../reports/widgets).
...
-  `recordsFilter` (object) -- фильтры для выборки записей каталога, такие как filter, sort и другие. подробнее в описании параметров [получения записей](./../data/records).
