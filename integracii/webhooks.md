...
Настройка вебхуков в Бипиуме описана в статье «[Вебхуки](../bpium-setup/systemcatalogs/upravlenie/webhooks.md)».
...
<table><thead><tr><th width="215.45458984375">Код</th><th>Описание</th></tr></thead><tbody><tr><td><code>record.created</code></td><td>После создания записи</td></tr><tr><td><code>record.updated</code></td><td>После изменения записи</td></tr><tr><td><code>record.deleted</code></td><td>После удаления записи</td></tr><tr><td><code>record.before.created</code></td><td>Перед созданием — ожидает одобрения</td></tr><tr><td><code>record.before.updated</code></td><td>Перед изменением — ожидает одобрения</td></tr><tr><td><code>record.before.deleted</code></td><td>Перед удалением — ожидает одобрения</td></tr><tr><td><code>record.updating</code></td><td>После изменения поля в карточке, до сохранения записи</td></tr></tbody></table>
...
* `values` — измененные значения полей в формате метода API на [получение записи](api/data/records.md)
...
* `values` — измененные значения полей в формате метода API на [получение записи](api/data/records.md)
* `prevValues` — предыдущие значения всех полей в формате метода API на [получение записи](api/data/records.md)
...
* `values` — измененные значения полей в формате метода API на [получение записи](api/data/records.md)
* `allValues` — все значения полей в формате метода API на [получение записи](api/data/records.md)
...
Если Бипиум получит ответ `410`, он удалит вебхук.
...
**Примечание**: Значения полей `user`, `object` и `file` должны быть переданы со всеми дополнительными параметрами. У каждого из этих полей имеется свой формат, как при [получении записи](api/data/records.md).
...
Список вебхуков хранится в каталоге «[Вебхуки](../bpium-setup/systemcatalogs/upravlenie/webhooks.md)» в отделе «Управление». Это значит, что для получения вебхуков и создания новых можно использовать стандартные методы [API Бипиума](api/).
